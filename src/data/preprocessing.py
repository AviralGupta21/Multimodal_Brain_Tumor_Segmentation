...
def paper_roi_detection(flair_slice):
    ...
    step3 = hat_transform(flair)
    step4 = big_median_filter(step3)
    step5 = multiply_with_flair(step4, flair)
    step6 = background_subtraction(flair)
    step7 = remove_background(step5, step6)
    step8 = enhance_roi(step7, flair)
...
def process_year(year):
    ...
    out_img = os.path.join(OUT_DIR, f"{year}_processed/images")
    out_msk = os.path.join(OUT_DIR, f"{year}_processed/masks")
